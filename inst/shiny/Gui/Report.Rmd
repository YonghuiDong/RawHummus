...
Robust and reproducible data is essential to ensure confidence in analytical results and is particularly important for large-scale metabolomics studies. Therefor raw data need to be inspected before data processing and statistical analysis in order to detect measurement bias and verify system consistency. In particular, proper quality control (QC) checks are of paramount importance in liquid chromatography mass spectrometry (LCMS) based metabolomics studies to ensure reliable and comparable results within experimental measurements [1-2].
...
if(is.data.frame(corTIC)){
  as.data.frame(round(cor(corTIC), 2)) %>%
...
autoTarget <- autoTarget %>%
  ...
  mutate(id = factor(id, levels = levels(id)[order(nchar(levels(id)))])) # order by RT, not perfect
...
if(nrow(msdata$MS2) == 0){
  ...
  TableMS2Pmass <- as.data.frame(round(cor(corPmass), 2)) %>%
    dplyr::mutate_all(~ kableExtra::cell_spec(.x, background = ifelse(.x < 0.85 , "#ff8080", "#FFFFFF"))) %>%
    ...
    kableExtra::scroll_box(width = "100%")
      ...
      ## correlation table
      TableMS2Pmass <- as.data.frame(round(cor(corPmass2), 2)) %>%
        dplyr::mutate_all(~ kableExtra::cell_spec(.x, background = ifelse(.x < 0.85 , "#ff8080", "#FFFFFF"))) %>%
        kableExtra::kbl(escape = FALSE, 
  ...
  ##2.1 plot precursor ions across RT range. Split subplot for better visulization for large data files.
  MS2_RT <- MS2unique %>% 
  split(ceiling(group_indices(., filename)/nsub)) %>% 
  ...
  corPRT <- MS2unique %>%
  mutate(rt_bins = cut(rt, breaks = floor(max(rt)/0.5))) %>% ## bin by 0.5 min
  group_by(filename) %>%
  count(rt_bins) %>%
  select(n, filename) %>%
  unstack()
  ...
  TableMS2PRT <- as.data.frame(round(cor(corPRT), 2)) %>%
    dplyr::mutate_all(~ kableExtra::cell_spec(.x, background = ifelse(.x < 0.85 , "#ff8080", "#FFFFFF"))) %>%
    ...
    kableExtra::scroll_box(width = "100%")
      ...
      ## correlation table
      TableMS2PRT <- as.data.frame(round(cor(corPRT2), 2)) %>%
        dplyr::mutate_all(~ kableExtra::cell_spec(.x, background = ifelse(.x < 0.85 , "#ff8080", "#FFFFFF"))) %>%
        kableExtra::kbl(escape = FALSE, 
...
Pearson correlation is used to quantify the similarity of precursor distribution across mass range plots among QC samples. Pearson correlation coefficient (R) over 0.85 indicate high similarity. If the value is below 0.85, it will be highlight in red in Table 5. 
...
Pearson correlation is used to quantify the similarity of precursor distribution across RT plots among QC samples. Pearson correlation coefficient (R) over 0.85 indicate high similarity. If the value is below 0.85, it will be highlight in red in Table 6. 

> **Note that** RT were binned by 0.5 min for Pearson correlation calculation.
