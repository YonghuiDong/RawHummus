...
**Summed TIC plot** is another quick-and-dirty way to overview global ion intensity variations among QC samples. It summed TIC across the entire points (scans) from the analysis. 
...
`Max Intensity Ratio`: is the maximum ion intensity variation. The value closer to 1 suggests that the ion intensity is stable. If the maximum intensity ratio is over <span style = "color: #ff8080">**2**</span>, the value will be highlight in <span style = "color: #ff8080">**red**</span> in Table 2.
...
if(nrow(msdata$MS2) == 0){
  ...
  ## plot precursor ion across mz range. Split subplot for better visulization for large data files.
  ...
  ## here nsub is used to better split the plot
  nfile <- length(levels(as.factor(MS2unique$filename)))
  nsub <- ifelse(nfile <= 8, 8, ifelse(ceiling(nfile/2) < 8, ceiling(nfile/2), 8))
  ...
  ## plot precursor ions across RT range. Split subplot for better visulization for large data files.
